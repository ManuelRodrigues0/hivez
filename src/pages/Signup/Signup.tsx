...
// Seamless Double-Buffered Chroma-Key Video Renderer to eliminate cut/lag between loops
function BeeChromaVideo() {
  ...
  useEffect(() => {
    ...
    const processFrame = () => {
      // Seamless crossfade/switch buffer right before video ends to avoid cutting black frames
      if (activeVideo.duration && activeVideo.currentTime >= activeVideo.duration - 0.15) {
      ...
      if (activeVideo.readyState >= 2) {
        const nativeW = activeVideo.videoWidth || 320;
        const nativeH = activeVideo.videoHeight || 180;
        ...
        offCtx.drawImage(activeVideo, cropX, cropY, cropW, cropH, 0, 0, targetW, targetH);
      ...
      if (bee) {
        if (!isMobile) {
          ...
          let currentPos = (bee as any).__lockedPos || {
            ...
            y: 135, // Positioned right beside the headline text as marked in your photo
          };
